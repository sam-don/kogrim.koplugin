...
-- Returns a translation function that checks kogrim .po files first,
...
--   1. Copy locale/kogrim.pot -> locale/<lang>.po (e.g. locale/es.po)
...
if lang ~= "en" and lang ~= "en_US" then
    local function try(name)
        ...
        if t and next(t) then
            ...
            logger.dbg("kogrim i18n: loaded " .. path .. " -- " .. n .. " strings")
            return t
        end
    end
    ...
    if translations then
        logger.dbg("kogrim i18n: installed for language: " .. lang)
    end
end

--- Translation function: checks kogrim .po first, then KOReader gettext.
...
-- ngettext: kogrim doesn't currently use plural forms, but keep the
